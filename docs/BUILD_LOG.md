...
The GitHub triage reduced 164 alerts to 34 by re-locking inside the
SDK 52 ranges, and showed the remaining 34 could only clear by crossing
the Expo SDK major, because the scaffold pins transitive versions, so hand-bumping
fights the SDK. The upgrade itself was left as recorded planned work.

## Phase 4: production completion (2026-09-18)
...
- **Real-run verification:** `expo export --platform web` builds all
  ...
  full intrinsic size because its style was never applied; it was fixed and
  re-verified in the same run.
- **Dependabot:** the aligned SDK 57 lockfile is what the remaining 34
  alerts were gated on; Dependabot re-scans after the push.
